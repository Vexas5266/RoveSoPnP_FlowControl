...
void Gantry::setGlobalPosition(float x, float y)
{
    ...
    m_grbl->sendCommand("G90 G0 X" + std::to_string(m_global_position.x) + " Y" + std::to_string(m_global_position.y));
}
...
void Gantry::setHeadHeight(float z)
{
    ...
    m_grbl->sendCommand("G90 G0 Z" + std::to_string(m_global_position.z));
}
